...
### Rapor: tam görsel yenileme — dataviz skill'e göre (2026-08-16, devam 12)

Kullanıcı önceki turda ("devam 10", madde 5) bilinçli olarak SINIRLI bir düzeltme yapılmasını istemişti (körü körüne tam yenileme değil, somut bir sorun — M-flag — bulunup düzeltilmişti). Bu turda kullanıcı **açıkça tam yenileme istedi**: *"PDF RAPORDAKİ 13/50 gibi kısımları kaldır yüzcde olarak gidelim raporun görünümünü düzenle... çok iyi görsellik istiyorum senden bu konuda pdf raporu sağlam bir el at."* Bu, önceki turun kısıtını genişleten, açık ve ayrı bir yetki.

**Hazırlık:** Anthropic'in paket `dataviz` skill'i çağrıldı (`references/palette.md` + `components.md`) — doğrulanmış ink/chrome token seti, sabit status paleti (good/warning/serious/critical), tabular-nums kuralı ("hero stat rakamları PROPORTIONAL kalır, sadece hizalanması gereken tablo sütunları tabular-nums alır — ilk taslakta bunu ters yapmıştım, düzeltildi") elde edildi.

**Bilinçli olarak DOKUNULMAYAN tek şey: ısı haritası renk skalası.** `_score_to_report_color()` (app.py) kırmızı→turuncu→sarı→yeşil "trafik ışığı" skalası kullanıyor — dataviz skill'in "sequential magnitude tek ton olmalı (rainbow değil)" kuralına teknik olarak aykırı. Ama bu skala canlı Matrix'in kendi `_SCORE_STOPS`'uyla (app.js) BİLEREK aynı tutuluyor (raporun kendi metni bile "ekrandaki harita ile aynı ölçek" diyor) — kullanıcı canlı Matrix'in renklerini task #51-61 boyunca onlarca turda "cırtlak"/"soluk" diye eleyerek özenle ayarlamıştı (bkz. #62 hâlâ açık: "Renk paleti seçenekleri göster, kullanıcıyla birlikte seç"). Rapor ile canlı ekran arasında renk-dili tutarlılığı, soyut palet-saflığından daha güçlü bir değer — bu yüzden yalnızca ısı haritasının ETRAFINDAKİ krom (kolon başlığı, kenarlık, legend) yeni tokenlara taşındı, hücre renklerinin kendisi hiç değişmedi.

**Uygulanan değişiklikler (`templates/report.html`, tam yeniden yazım — `app.py` route/veri katmanına DOKUNULMADI):**
1. **"13/50" deseni iki yerde bulundu, ikisi de düzeltildi** — (a) Taktik Bazlı Kapsama tablosunda `{{ t.covered }} / {{ t.total }}` sütunu tamamen silindi (zaten yanındaki bar+yüzde sütunlarıyla üç kere aynı bilgiyi tekrarlıyordu); (b) Yönetici Özeti'ndeki "Ana Teknik" kartının alt yazısı `detected_subtechniques / total_subtechniques` kesri de aynı desendi, yüzdeye çevrildi. Kural: hiçbir yerde `{{ a }} / {{ b }}` kesir biçimi kalmayacak — destekleyici ham sayılar düz cümle içinde kalabilir ("132 teknikte tespit yok"), kesir olarak DEĞİL.
2. **5 kart tamamen %-öncelikli yeniden tasarlandı** (etiket üstte → büyük değer ortada → destek cümlesi altta, klasik "stat tile" sırası). Tespit/Kapsamsız/Mitigation kartları artık büyük rakam olarak YÜZDE gösteriyor, ham sayı küçük destek cümlesine indi. Kapsamsız% hesap yöntemi bilinçli seçildi: `100 - coverage_pct` (bağımsız `uncovered/total` yeniden hesabı DEĞİL) — böylece Tespit%+Kapsamsız% ekranda her zaman TAM 100.0 gösterir, iki ayrı yuvarlamadan kaynaklanan 99.9/100.1 görünümü riski yapısal olarak imkânsız.
3. **Status renkleri sabitlendi:** kart üst kenarlığı+değer rengi Tespit=`--status-good`(#0ca30c)/Kapsamsız=`--status-critical`(#d03b3b), nötr kartlar (Ana Teknik/Mitigation/Ortalama Skor)=`--brand`. Genel Durum kutusu 3 kademesi (İyi/Orta/Yetersiz) sabit palete taşındı — "Orta" için ham `warning` (#fab219) DEĞİL `serious` (#ec835a) kullanıldı (palette.md'nin kendi notu: warning açık yüzeyde 1.79:1 kontrastla metin/ince çizgi için riskli; serious 2.57:1 ile kenarlık için daha güvenli, üç kademeyi good→serious→critical'e sadeleştirdi).
4. **Tablo kroması yenilendi:** başlık `background:var(--page)` + `border-bottom:2px solid var(--brand)` (eski dört tablo boyunca tekrarlanan düz mavi banttan daha az mürekkep/toner, daha "premium" his), zebra stripe (`tbody tr:nth-child(even)`), `td{font-variant-numeric:tabular-nums}` (sayı hizası) — ama kart hero rakamları BİLEREK proportional kaldı (dataviz kuralı, madde yukarıda).
5. **`print-color-adjust:exact` + `-webkit-` öneki eklendi** (`html` + `body` + `@media print` içinde tekrar) — bu GERÇEK bir teknik düzeltme, sadece estetik değil: bu olmadan Chrome/Edge'in "Yazdır/PDF Kaydet" akışı, kullanıcı yazdırma diyaloğunda "arka plan grafikleri"ni elle işaretlemediği sürece ısı haritası hücre renklerini ve rozet arka planlarını SESSİZCE beyaza çevirebiliyordu — tam da bu raporun asıl işlevini (renk kodlu harita) baltalayabilecek bir kusurdu, kullanıcı fark etmeden.
6. Section title'lara küçük mavi vurgu çubuğu (`::before`, 4×14px), bölüm notu genişliği sınırlandı (`max-width:920px`, okunabilirlik), rozetler (`.p-1..4`, `.s-*`) aynı token setine taşındı — yeşil metin `--success-text`(#006300, palette.md'nin kendi "delta good" token'ı, ham `--status-good`'dan daha koyu/okunaklı küçük metin için).

**Doğrulama (bu ortamda ekran görüntüsü yine çalışmıyor — DOM/CSSOM ile yapıldı):** sayfa Jinja hatası vermeden render oldu (route'taki `try/except` yalnızca veri hesaplamasını sarıyor, template hatası olsaydı 500 dönerdi — dönmedi). `get_page_text` ile tam gövde okundu: hiçbir yerde "/" kesir deseni kalmadı, Tespit%(40.5)+Kapsamsız%(59.5)=100.0 tam eşleşiyor. `getComputedStyle` ile doğrulanan: `--brand`/`--status-good` custom property'leri doğru çözülüyor, `.rpt-card.good` kenarlığı `rgb(12,163,12)`, Genel Durum kutusu gerçek veriyle (`pct=40.5`) doğru şekilde `warn` sınıfını alıp `serious` rengini kullanıyor, tablo `th` arka planı/alt çizgisi yeni tokenlara eşleşiyor, `td` tabular-nums AÇIK ama kart hero değeri (`.rpt-card-val`) tabular-nums KAPALI (doğru ayrım), section-title `::before` 4×14px render ediyor, zebra satır rengi doğru, 254 matris hücresi ve 5 kart eksiksiz sayıda DOM'da, body font system-ui yığınına döndü, konsolda hata yok, yatay taşma yok (`scrollWidth==clientWidth`). `_score_to_report_color()`/`_SCORE_STOPS` hiç değiştirilmedi — canlı Matrix ile ısı haritası rengi hâlâ birebir aynı skala.

Not: `templates/report.html` `?v=` versiyonlamasına dahil değil (bağımsız route, `index.html`'in script/style cache-bust mekanizmasının dışında) — versiyon bump gerekmedi.
